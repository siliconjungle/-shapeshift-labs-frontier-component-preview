# @shapeshift-labs/frontier-component-preview

Frontier-native component preview books for apps that want generated previews, state-aware variants, Vite dev integration, browser inspection, fuzzing, benchmarks, and agent-readable evidence without adopting a separate preview framework.

- npm: [`@shapeshift-labs/frontier-component-preview`](https://www.npmjs.com/package/@shapeshift-labs/frontier-component-preview)
- source: [`siliconjungle/-shapeshift-labs-frontier-component-preview`](https://github.com/siliconjungle/-shapeshift-labs-frontier-component-preview)
- license: MIT
...
```sh
npm install @shapeshift-labs/frontier @shapeshift-labs/frontier-component-preview
```

## Usage

Define normal TSX/JSX components. The preview package can discover exported components and generate a preview book without hand-written stories.

```tsx
// src/components/TodoCard.tsx
export function TodoCard(props: { title: string; done?: boolean }) {
  return <article data-done={props.done}>{props.title}</article>;
}
```

```sh
npx frontier-preview build --cwd . --out .frontier/preview --json
```

For explicit previews, keep the data serializable:

```ts
import { defineFrontierPreview } from '@shapeshift-labs/frontier-component-preview';

export default defineFrontierPreview({
  component: 'TodoCard',
  source: { file: 'src/components/TodoCard.tsx', exportName: 'TodoCard' },
  args: [
    { name: 'title', type: 'text', defaultValue: 'Ship preview package' },
    { name: 'done', type: 'boolean', defaultValue: false }
  ],
  state: [
    { kind: 'frontier-state', path: '/entities/todos/t1', initial: { title: 'Ship preview package', done: false } }
  ],
  variants: [
    { id: 'default', title: 'Default' },
    { id: 'done', title: 'Done', args: { done: true } }
  ]
});
```
...
```ts
import { frontierComponentPreviewVite } from '@shapeshift-labs/frontier-component-preview/vite';

export default {
  plugins: [
    frontierComponentPreviewVite({
      routePrefix: '/__frontier_preview'
    })
  ]
};
```

By default the Vite integration, auto-discovery, browser book, floating inspector, state bridge, CRDT timeline hooks, event-log replay hooks, telemetry evidence, fuzz plans, and benchmark plans are enabled. Each integration stays optional:
...
const tests = createPreviewHarnessManifest(manifest);
const fuzzCases = createPreviewFuzzCases(manifest);
const browserEvidence = createPreviewBrowserEvidencePlan(manifest);
```

The generated harness is meant to plug into `frontier-test`, `frontier-playwright`, `frontier-inspect`, `frontier-trace`, and `frontier-logging` through structural manifests. It keeps preview routes, DOM probes, state paths, telemetry events, interaction replay, fuzz cases, and benchmark targets tied to the same component graph.

## Boundary

The package root depends only on `@shapeshift-labs/frontier`. Node filesystem discovery, Vite hooks, browser preview rendering, and harness helpers are isolated behind subpath exports. It does not implement compatibility layers for external preview frameworks; this package is the Frontier preview system.
...
The package benchmark measures manifest normalization, search indexing, harness-plan generation, browser book rendering, and generated fuzz-case creation for Frontier preview fixtures.
